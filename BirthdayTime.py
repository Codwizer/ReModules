...
d_msg = ['Ждешь его?', 'Осталось немного)', 'Дни пролетят, даже не заметишь', 'Уже знаешь что хочешь получить в подарок?)', 'Сколько исполняется?', 'Жду не дождусь уже']
...
class DaysToMyBirthday(loader.Module):
    ...
    async def btname(self, message):
        ...
        if self.db.get(__name__, 'change_name'):
            ...
            await message.client(UpdateProfileRequest(last_name=self.db.get(__name__, 'last_name')))
