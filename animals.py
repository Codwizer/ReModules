...
class animals(loader.Module):
    ...
    async def fcatcmd(self, message: Message):
        ...
        await utils.answer_file(message, cat_url, self.strings("done"), force_document=True)
    ...
    async def fdogcmd(self, message: Message):
        ...
        await utils.answer_file(message, dog_url, self.strings("done"), force_document=True)
    ...
    async def catcmd(self, message: Message):
        ...
        response = requests.get('https://api.thecatapi.com/v1/images/search')
        cat_url = response.json()[0]['url']
        await utils.answer_file(message, cat_url, self.strings("done"), force_document=False)
    ...
    async def dogcmd(self, message: Message):
        ...
        response = requests.get('https://api.thedogapi.com/v1/images/search')
        dog_url = response.json()[0]['url']
        await utils.answer_file(message, dog_url, self.strings("done"), force_document=False)
